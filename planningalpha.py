...
def plotexpectedreturn(tstepsNO,expectedreturnmodel,expectedreturn,optimalreturn):
    ...
    plt.xlabel('Epoch')
    plt.ylabel(r'$\mathbb{\rm E}(\hat{G})$')
    plt.plot(np.linspace(-1,tstepsNO+2,100),np.ones(100)*optimalreturn)
    plt.xlim([-1,tstepsNO+1])
    plt.legend([r'$\mathbb{\rm E}(\hat{G})$ w.r.t model $\mathbf{M}_{t}$',r'$\mathbb{\rm E}(\hat{G})$ w.r.t true $\mathbf{M}_{t}$',r'Optimal $\mathbb{\rm E}(\hat{G})$'])
...
if __name__ == '__main__':
    ...
    savemodel = np.zeros((10,11))
    savetrue = np.zeros((10,11))
    #for i in range(10):
    ...
    plotexpectedreturn(tstepsNO, expectedreturnmodel, expectedreturn,optimalreturn)
        # savemodel[i] = expectedreturnmodel
        # savetrue[i] = expectedreturn
        
        # expectedreturn = np.mean(savetrue,axis=0)
        # expectedreturnmodel = np.mean(savemodel,axis=0)
        # plotexpectedreturn(tstepsNO, expectedreturnmodel, expectedreturn,optimalreturn)
        
    
    # plt.figure()
    # for i in range(10):
    #     plt.plot(range(tstepsNO+1),savemodel[i],'r')
    #     plt.plot(range(tstepsNO+1),savetrue[i],'b')
    # plt.xlabel('Epoch')
    # plt.ylabel(r'$\mathbb{\rm E}(\hat{G})$')
    # plt.plot(np.linspace(-1,tstepsNO+2,100),np.ones(100)*optimalreturn)
    # plt.xlim([-1,tstepsNO+1])
